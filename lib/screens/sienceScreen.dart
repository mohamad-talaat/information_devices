  ...
  Widget build( context ) {
    return BlocConsumer<newsAppCubit , newsAppStates>(
      builder: (context, state) {
        ...
        return Scaffold(
            body:ConditionalBuilder(condition: state is! newAppScienceLoadingState,
                fallback:(context) => Center(child: CircularProgressIndicator()),
                builder: (context) {
                  return ListView.separated(itemBuilder: (context, index)=>ANANA(newsAppCubit.get(context).science[index] //to call on (var business=[];)from Cubit ((the place where carry the data))
